import math

# Variant 1
# Small exponent attack variant 1
TEST_CASE = (
    [ # C_VALUES
        0x9f18cb5a10b43cb08f799bb599e6f1efb981c9a5cd352793715c5204e879ef37,
        0x4228bc1401f6deb12f8235f67779d5f4429b76c42d176b17a8d636a173f1364f,
        0x42ff76fd7459f6fb63bffb9380467b6822fa1ee7d6843c06d55b75f01776c9ce,
    ],
    [ # N_VALUES
        0xD7E0B1B02BCE1C6E2213595E70350EFD3B45980ACC354447E43A31277C775A25,
        0xABDDDA209D6C7583D4D48DE609C7DAAA13ACC17222765058D0BA5D85A689EB87,
        0xB85BB9166C83050778CD2EE9AD9BBEB1826387952C36A1887CAE5CC1028C44F9,
    ],
    3 # E_VALUE
)

TASK = (
    [ # C_VALUES
        ,
        ,
        ,
        ,
        ,
    ],
    [ # N_VALUES
        ,
        ,
        ,
        ,
        ,
    ],
    5 # E_VALUE
)

# Compute n-th root of a, using binary search
def nth_root(a: int, n: int) -> int:
    low = 1
    high = a

    while low <= high:
        mid = (low + high) // 2
        mid_exp = pow(mid, n)

        if mid_exp == a:
            return mid
        elif mid_exp < a:
            low = mid + 1
        else:
            high = mid - 1

# Use Chinese remainder theorem to find a solution of the system:
#       x_1 = a_1 (mod n_1)
#       x_2 = a_2 (mod n_2)
#       ...
#       x_k = a_k (mod n_k)
# in form x mod N, where N = n_1  * n_2 * ... * n_k
def chinese_remainder_theorem(a_values: list, n_values: list) -> int:
    k = len(a_values)
    if len(n_values) != k:
        raise ValueError("Paramethers must be of the same size.")
    
    N = 1
    for n in n_values:
        N *= n

    x = 0
    for a, n in zip(a_values, n_values):
        M_i = N // n
        N_i = pow(M_i, -1, n)
        x += a * M_i * N_i
    
    return x % N

def small_exponent_attack(c_values: list, n_values: list, e: int):
    C = chinese_remainder_theorem(a_values=c_values, n_values=n_values)
    M = nth_root(a=C, n=e)

    return M

def meet_in_the_middle(C: list, n: list, e: int) -> int:

    l = int(math.log(n, 2) / 65) #Counting l
    temp_l = int(pow(2, l/2))

    #creating X pairs
    X = []
    for i in range(1, temp_l):
        X.append((i, int(pow(i, e, n))))

    #Filling C_s
    C_s = []
    for i in range(1, temp_l):
        C_s.append(C * pow(i, -1 * e, n))
        for j in X:
            if C_s[i - 1] == j[1]:
                M = pow(j[1], -e, n) * pow(i, e, n)
                return M

    return "Open text was not defined"

def main():
    C_c = 
    N_n = 
    e_e = 65537

    # c_values, n_values, e = TEST_CASE
    c_values, n_values, e = TASK
    M = small_exponent_attack(c_values=c_values, n_values=n_values, e=e)

    print(f"Message: {hex(M)}")
    print()
    print(f"Decoded message: {hex(M)[(2 + 1 + 2*8 + 2):]}")

    M_m = meet_in_the_middle(C = C_c, n = N_n, e = e_e)
    print(f"Decoded message for MitM attack:", M_m)


if __name__ == "__main__":
    main()

